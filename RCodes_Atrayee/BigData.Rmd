...
```{r}
items_sold <- count(bg2, c("Time", "Target")) # Trying to find interesting items which were sold multiple times.
items_bought <- count(bg3, c("Time", "Target")) # Trying to find interesting items which were bought multiple times.
...
items <- count(bg23, c("Time", "Target"))
max(items$freq)
items <- items %>% filter(items$freq >=2)

frequent_items <- items$Target
length(frequent_items)
...
```{r}
multi_sources <- count(transactions, c("Target", "Source"))
