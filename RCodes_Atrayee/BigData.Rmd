...
```{r}
location_clusters <- count(bg6, c("Source"))
nrow(location_clusters)
min(location_clusters$freq)
max(location_clusters$freq)
location_clusters <- location_clusters %>% filter(location_clusters$freq >=10 )
...
```{r}
for (i in time) {
  target1 <- bg6 %>% filter(Time == i)
  if(length(target1) < 3){
    bg6 <- bg6 %>% filter(Time!=i)
...
```{r}
target1 <- bg6 %>% filter(Time == 20413606)
target2 <- bg6 %>% filter(Target==509607)
target3 <- bg6 %>% filter(Target==561157 & Time == 52 & SourceLocation==0)
...
```{r}
ggplot(target3, aes(x=as.character(Source), y=Time, color = as.character(SourceLocation), fill=as.character(SourceLocation))) + 
