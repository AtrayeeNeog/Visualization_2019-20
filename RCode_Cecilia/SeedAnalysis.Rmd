...
#get all datasets of Seed1 for the Network Analysis
Seed1_Comm <- read.csv('Seed1-Graph2.csv')
Seed1_Others <- read.csv('Seed1-Graph2NonCom.csv')
Seed1_CommC = select(Seed1_Comm, -X)
Seed1_OtherC = select(Seed1_Others, -X)
Seed1_AllC = full_join(Seed1_CommC,Seed1_OtherC)
...
nodes_S1 <- full_join(Sources, Targets, by = "label")
nodes_S1
nodes_S1 <- nodes_S1 %>% rowid_to_column("id")
nodes_S1
...
edges_Seed1 <- per_route_Seed1 %>% 
  left_join(nodes_S1, by = c("Source" = "label")) %>% 
  rename(from = id)

edges_Seed1 <- edges_Seed1 %>% 
  left_join(nodes_S1, by = c("Target" = "label")) %>% 
  rename(to = id)
...
# Creating a Network:
routes_network_Seed1 <- network(edges_Seed1, vertex.attr = nodes_S1, matrix.type = "edgelist", ignore.eval = FALSE)
...
# Graph3 Extended Network Building:
...
plot(routes_network_S3,vertex.cex = 1, main = "Seed3 All Channels")
